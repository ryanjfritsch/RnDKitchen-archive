...
export default class Carousel extends Component {
  ...
  updateCarouselCard() {
    this.interval = setTimeout(() => {
      this.setState({
        ...
        currentLink: this.state.allMealData[this.state.currentMealIndex].node
          .fields.slug,
      })
    }, 500)
  }
  ...
  updateCarouselTable(slotToChangeNum) {
    this.interval = setTimeout(() => {
      if (slotToChangeNum === 0) {
        this.setState({
          mealSlot0: this.state.allMealData[this.state.nextMealIndex],
        })
      } else if (slotToChangeNum === 1) {
        this.setState({
          mealSlot1: this.state.allMealData[this.state.nextMealIndex],
        })
      }
    }, 1000)
  }
  ...
  render() {
    const meal0Image = require('../images/carousel/' +
      this.state.mealSlot0.node.frontmatter.shortName +
      '.png')
    const meal1Image = require('../images/carousel/' +
      this.state.mealSlot1.node.frontmatter.shortName +
      '.png')
            ...
            <div id="card">
              <div className="cardContainer" id="cardContainer1">
                <div className="cardHeader">
                  <span className="carouselCardTitle">
                    {this.state.carouselCardMeal.node.frontmatter.title}
                  </span>
                  <span className="carouselCardDate">
                    {this.state.carouselCardMeal.node.frontmatter.date}
                  </span>
                  <StarRating
                    rating={this.state.carouselCardMeal.node.frontmatter.rating}
                  />
                </div>
                <div className="foodInformation">
                  <div className="metricRow">
                    <div className="foodInfoItem">
                      ...
                      <span>
                        {this.state.carouselCardMeal.node.frontmatter.prepTime}
                      </span>
                    </div>
                    <div className="foodInfoItem">
                      <img src={require('../images/spatulaX.png')} alt="" />
                      <span>
                        {
                          this.state.carouselCardMeal.node.frontmatter
                            .difficulty
                        }
                      </span>
                    </div>
                  ...
                  <div className="metricRow">
                    ...
                    <div className="foodInfoItem">
                      <img src={require('../images/flame.png')} alt="" />
                      <span>
                        {this.state.carouselCardMeal.node.frontmatter.heatLevel}
                      </span>
                    </div>
                  </div>
                </div>
